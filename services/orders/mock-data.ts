...
import { Order, OrderStatus, Quote, TimelineEvent } from "../../core/types/orders.types";
...
export const generateTimeline = (status: OrderStatus, createdAt: string): TimelineEvent[] => {
  const timeline: TimelineEvent[] = [{ status: OrderStatus.PENDING_PAYMENT, timestamp: createdAt }];
...
export const getOrdersByRole = (role: "user" | "ally" | "admin", userId: string): Order[] => {
  switch (role) {
    ...
    case "ally":
      return mockOrders.filter(order => order.userId === userId || order.allyId === userId);
