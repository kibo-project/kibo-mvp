...
export class AuthService {
  ...
  async login(token: string) {
    ...
    const jwtToken = await generateToken(user.id!, role);
  ...
  async changeUserRole(userId: string, roleIdNew: string) {
    const roleName = await this.usersRepository.getRoleNameByRoleId(roleIdNew);
    ...
    const userChanged = await this.usersRepository.updateUser(userId, roleIdNew);
  ...
  async changeUserRole2(userId: string, roleId: string) {
    ...
    const roleName = await this.usersRepository.getRoleNameByRoleId(userId);
    const jwtToken = await generateToken(userId, roleName);
